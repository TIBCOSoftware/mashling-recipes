...
function testcase1 {
mashling-gateway -c smart-circuit-breaker-gateway.json > /tmp/circuit1.log 2>&1 &
pId=$!
sleep 5
...
if [ $response -eq 200 ] && [ $response1 -eq 200  ] && [ $response2 -eq 403 ] &&  [[ "echo $(cat /tmp/circuit1.log)" =~ "Code identified in response output" ]] && [[ "echo $(cat /tmp/test.log)" =~ "circuit breaker tripped" ]]
    then 
        echo "PASS"
    else
        echo "FAIL"
