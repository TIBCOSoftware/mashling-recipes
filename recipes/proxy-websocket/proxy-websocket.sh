...
# Run one client
function testcase1 {
cat > $GOPATH/password.txt <<EOL
foo:bar
tom:jerry
EOL
export BASIC_AUTH_FILE=$GOPATH/password.txt
...
pId1=$!
sleep 5
go run main.go -client -name=CLIENT1 -url=ws://localhost:9096/ws -basicauth=foo:bar > /tmp/client1.log 2>&1 & pId2=$!
