...
pId=$!
sleep 15
curl -X PUT "http://foo:bar@localhost:9096/pets" -H "accept: application/xml" -H "Content-Type: application/json" -d '{"category":{"id":120,"name":"Animals"},"id":120,"name":"SPARROW","photoUrls":["string"],"status":"sold","tags":[{"id":0,"name":"string"}]}'
response=$(curl --request GET http://foo:bar@localhost:9096/pets/120 --write-out '%{http_code}' --silent --output /dev/null)
kill -9 $pId
if [ $response -eq 200  ] 
    then 
        echo "PASS"
...
pId=$!
sleep 15
response=$(curl --request GET http://tom:jerry@localhost:9096/pets/120 --write-out '%{http_code}' --silent --output /dev/null)
kill -9 $pId
if [ $response -eq 200  ] 
    then 
        echo "PASS"
